...
def _build_atari_remix(spotify_url: str, bars: int = 16) -> dict:
    ...
    for step in range(total_steps):
        ...
        if step_in_bar in (0, 8) or (step_in_bar == 12 and rng.random() < 0.28):
        ...
        if step_in_bar in (2, 6, 10, 14):
            melody_offsets = [12, 10, 7, 14, 15, 12]
...
def atari_preview():
    ...
    try:
        remix = _build_atari_remix(spotify_url, bars=bars)
    ...
    return Response(
        remix["mix_wav"],
        mimetype="audio/wav",
        headers={"Content-Disposition": f'inline; filename="{filename}"', "X-Atari-BPM": str(remix["bpm"])},
    )


@app.post("/api/atari/export")
def atari_export():
    payload = flask_request.get_json(silent=True) or {}
    spotify_url = (payload.get("spotify_url") or flask_request.form.get("spotify_url") or "").strip()
    bars_value = payload.get("bars") or flask_request.form.get("bars") or "16"
    try:
        bars = int(bars_value)
    except ValueError:
        return jsonify({"error": "bars must be an integer"}), 400
    ...
    with io.BytesIO() as zip_buf:
        with zipfile.ZipFile(zip_buf, "w", compression=zipfile.ZIP_DEFLATED) as zf:
            ...
            for stem_name in ATARI_TRACK_NAMES:
                wav_data = _pcm16_wav_bytes(remix["stems_float"][stem_name], remix["sample_rate"])
